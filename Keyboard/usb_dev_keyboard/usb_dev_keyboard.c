...
//
// Copyright (c) 2011-2019 Texas Instruments Incorporated.  All rights reserved.
...
//*****************************************************************************
//
// Sends Modifier Keys via the USB HID keyboard interface.
//
//*****************************************************************************
...
SendMod(modInt)
{
    ...
    //Win+R
    if (modInt==1){
        mod = HID_KEYB_RIGHT_GUI;
        key = HID_KEYB_USAGE_R;

    //Ctrl+C
    }else if (modInt==2){
        mod = HID_KEYB_LEFT_CTRL;
        key = HID_KEYB_USAGE_C;

    //Ctrl+V
    }else if (modInt==3){
        mod = HID_KEYB_LEFT_CTRL;
        key = HID_KEYB_USAGE_V;
    }

    else if (modInt==4){
        mod = 0;
        key = HID_KEYB_USAGE_KEPAD_ENTER;
    }
...
int getCmd (void){
    int retVal = (GPIO_PORTE_DATA_R >> 1) & 3;
    if (retVal == prev){
        return 0;
    }else{
        prev = retVal;
...
{
    PLLInit();
    SYSCTL_RCGCGPIO_R |= 0xFF; // enable clock for PORT F
            ...
            if (portVal)
            {
                ...
                else 
                {
                    ...
                    // Open video "JUST DO IT"
                        ...
                        SendMod(4);
                        SysTick_Wait10ms(30);
                        SendString("https://www.youtube.com/watch?v=ZXsQAXx_ao0");
                        SendMod(4);
                    } else if (portVal == 3) {
                        SendMod(1);
                        SysTick_Wait10ms(30);
