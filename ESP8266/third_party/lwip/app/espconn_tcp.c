...
void ICACHE_FLASH_ATTR espconn_tcp_memp_free(espconn_msg *pmemp)
{
	...
	os_printf("espconn_tcp_memp_free called, starting\n")
...
espconn_tcp_reconnect(void *arg)
{
	...
	if (precon_cb != NULL) {
		...
		os_printf("before espconn_tcp_reconnect memp_free\n");

		/*frees the connection memory*/
		espconn_tcp_memp_free(precon_cb);
		os_printf("after espconn_tcp_reconnect memp_free\n");
	} else {
...
espconn_tcp_disconnect_successful(void *arg)
{
	...
	if (pdiscon_cb != NULL) {
		...
		os_printf("before espconn_tcp_disconnect memp_free\n");

		/*frees the connection memory*/
		espconn_tcp_memp_free(pdiscon_cb);
		os_printf("after espconn_tcp_disconnect memp_free\n");
	} else {
